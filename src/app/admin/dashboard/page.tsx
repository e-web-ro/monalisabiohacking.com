                                ...
                                {Object.entries(data[activeLang].shop.products).map(([id, product]: [string, any]) => (
                                    <div key={id} className="bg-secondary/20 p-6 rounded-2xl border border-white/5 space-y-4 relative group">
                                        ...
                                        <div className="space-y-2 pt-2 border-t border-white/5">
                                            <label className="text-[10px] text-zinc-600 font-bold uppercase flex items-center gap-2">
                                                <ImageIcon className="w-3 h-3" /> Fișier Digital (PDF)
                                            </label>

                                            <div className="flex items-center gap-4">
                                                {product.file_url ? (
                                                    <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-3 py-2 rounded-lg text-emerald-500 text-xs">
                                                        <CheckCircle2 className="w-4 h-4" />
                                                        <span className="truncate max-w-[200px]">{product.file_url.split('/').pop()}</span>
                                                        <button
                                                            onClick={() => updateProduct(id, "file_url", "")}
                                                    ...
                                                    <span className="text-zinc-500 text-xs italic">Niciun fișier încărcat</span>
                                                )}
                                                ...
                                                <input
                                                    ...
                                                    onChange={async (e) => {
                                                        ...
                                                        try {
                                                            ...
                                                            updateProduct(id, "file_url", data.url);
                                                    ...
                                                    className="text-xs text-zinc-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-primary file:text-black hover:file:bg-emerald-400 cursor-pointer"
                                                />
                                            </div>
                                            <p className="text-[10px] text-zinc-600">
                                                * Acest fișier va fi trimis automat clientului pe email după plata cu succes.
                                            </p>
                                        </div>
                                    </div>
                                ))}
