...
export function ShopSection({ dict, lang }: ShopSectionProps) {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // Fallback to static dict if API fails
                const staticProducts = Object.entries(dict.shop.products).map(([id, product]: [string, any]) => ({
                    id,
                    ...product
                }));
    ...
    const handleCheckout = async (e: React.FormEvent) => {
        ...
        // We now skip step 2 (manual card entry) and go straight to Stripe
                                ...
                                <form onSubmit={handleCheckout}>
                                    <div className="p-8 border-b border-white/5 flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-zinc-500 text-sm mt-1">Informații livrare</p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-6">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div className="space-y-2">
                                                    ...
                                                    <input required name="firstName" type="text" className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" />
                                                </div>
                                                <div className="space-y-2">
                                                    <label className="text-sm text-zinc-400">Prenume</label>
                                                    <input required name="lastName" type="text" className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" />
                                                </div>
                                            </div>
                                            <div className="space-y-2">
                                                <label className="text-sm text-zinc-400">Email (pentru livrarea produselor)</label>
                                                <input required name="email" type="email" className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-primary transition-colors" />
                                            </div>

                                            <div className="p-4 rounded-xl border border-primary/20 bg-primary/5 flex items-center justify-between mt-6">
                                                <div className="flex items-center gap-3">
                                                    <CreditCard className="text-primary w-5 h-5" />
                                                    <span className="text-sm text-zinc-300">Total de plată:</span>
                                                </div>
                                    ...
                                    <div className="p-8 border-t border-white/5 flex items-center justify-between">
                                        ...
                                        <button
                                            type="submit"
                                            className="px-8 py-3 bg-primary text-black font-bold rounded-full hover:bg-emerald-400 transition-all flex items-center gap-2 group shadow-[0_0_20px_rgba(16,185,129,0.3)]"
                                        >
                                            Plătește cu Cardul <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                                        </button>
                                    </div>
                                </form>
                            ) : (
