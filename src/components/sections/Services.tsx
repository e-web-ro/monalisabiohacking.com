...
export function Services({ dict, lang }: ServicesProps) {
    // Convert dictionary items to array dynamically
    const services = Object.entries(dict.items).map(([key, item]: [string, any]) => ({
        id: key,
        title: item.title,
        description: item.description,
        duration: "1 h", // This might need to be in the dict too if customizable, fallback to 1h
        price: item.price,
        price_display: item.price === "0 €" ? "Gratuit" : item.price,
        period: item.period,
        highlight: key === 'reset', // Keep existing highlight logic or make it data-driven
        badge: item.badge,
